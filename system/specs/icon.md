...
- **default** — the glyph, in three sizes: `md` at `--spacing-md`, `lg` at `--spacing-lg`, `xl` at `--spacing-xl`. The `<svg>` carries `fill="currentColor"` — set by the template, not copied: `gen-icons` copies the path `d` and nothing else, so the committed map holds no `fill` at all — and the colour then reaches the glyph by inheritance, which is how it takes the colour of whatever it sits in and wears every pack without a rule of its own. An icon part is not a control — no hover, no focus, no pressed.
