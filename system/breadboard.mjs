...
function mount(root) {
  ...
  //
  // The whole tail morphs (#171), and all three statements stay inside the callback for a reason
  // each. render() must be in it or there is nothing to capture; applyPendingFocus runs as its last
  // step and focusing a not-yet-inserted node is a no-op. publish() must be in it because
  // BUILD_CHANGE dispatch is synchronous — putting it inside means ONE transition captures the
  // board, the pattern stage, the verdict and the keep rail moving together, which is the whole
  // point of morph.mjs's shared re-entrancy flag. announce() keeps its place at the end so the
  // render → publish → announce order is unchanged; [data-bb-live] sits outside the re-rendered
  // subtree (build.html), so the live region is not captured and still speaks during a transition.
  ...
  // Both connect verbs morph too (#171): entering and leaving connect mode adds or removes a
  // "Connect here" button in every eligible place, which reflows the board. No publish here — these
  // change the mode, not the board — so the transition captures this module's own subtree only.
  ...
  // Re-draft is a board verb the visitor pressed, so it morphs on commit()'s terms (#171): render
  // and publish inside one callback, announce last.
  function redraft() {
  ...
  function renderPlace(place, index) {
    ...
    // Each place is its own view-transition group, named by its board id (#171). render() rebuilds
    // every place element on every verb, so without this the browser has no way to know the new
    // "p2" is the old "p2" and the board would crossfade; with it, the places that SURVIVE an add
    // or a remove visibly slide to their new positions instead. Place ids are stable and unique
    // (nextId over the taken set, :124/:227) and an id is only freed once its element is gone, so
    // two places can never share a name inside one snapshot.
    //
    // THE ONE INLINE-STYLE WRITE ON THIS PAGE THAT IS NOT build-import's applyToStage, and it is
    // whitelisted by name in build-checks group 7. It is allowed because it is not a VALUE: no
    // visitor-supplied string reaches it. `place.id` is generated by this module's own nextId and
    // is always /^p\d+$/, and what it produces is a transition GROUP IDENTITY, which paints nothing
    // at rest. The matching view-transition-class is a static rule in build.html, because a
    // constant belongs in the stylesheet — this is the only part that genuinely has to be dynamic.
    section.style.viewTransitionName = `bb-place-${place.id}`;
