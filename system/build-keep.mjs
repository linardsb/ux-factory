...
function mount(root) {
  ...
  // NOTHING RIDES ALONG ANY MORE (#302). Until v3 a link could ARRIVE carrying the sender's grid
  // arrangement, and this page — which has no canvas and could neither show it nor edit it — passed
  // it back through untouched so a visitor who only came to rename a place did not silently flatten
  // it. `g` is retired with the grid, so the link carries the build and nothing else.
