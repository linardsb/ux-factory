...
- **Per-company bespoke builds:** post-MVP layer; architecture keeps it cheap by construction (packs + ledger). Layer architecture decided 2026-07-19 → [per-company-brief.architecture.md](./per-company-brief.architecture.md).
- **Legibility surfaces (annotated source + term definitions):** two patterns borrowed from codebase-to-course, implemented natively under the PRD's legibility bar; snippets build-time-extracted so shown code can't drift. Decided 2026-07-20 → [annotated-source-glossary.architecture.md](./annotated-source-glossary.architecture.md).
...
- [x] When the per-company layer lands: ledger → scenario-package unification? — settled 2026-07-19: unified (a company brief compiles to the same scenario-package shape); the layer's architecture decided in [per-company-brief.architecture.md](./per-company-brief.architecture.md).
