...
- **Stack & libraries.** No new runtime deps. Build-time: Node ESM + the portal's `@anthropic-ai/claude-agent-sdk` dependency (already used by `record-composition`/`probe-vision`). View-time: vanilla, no framework, no view-time LLM — unchanged. *Alternative considered:* a dedicated vision/codegen library for the ceiling engine — rejected, the Agent SDK already returns usable image blocks (probe-vision proved it) and adding a dep breaks the zero-dep-where-possible rule.
