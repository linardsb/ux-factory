...
1. **Derivation fidelity** *(gates the headline claim's labeling)*
   ...
   **Clean real-product test (2026-07-19) — Cantor8, the number the label rests on.** To escape both the closed-loop (Verdant) and brand-recall (Carbon) confounds, the read ran on an *unfamiliar* product — Cantor8's product page (captured + read locally, uncommitted). Ground truth came from the site's own rendered CSS (`--blue: #044ab3`, corroborated by its primary-CTA fill), not a guess. The capability read the accent as `#1E40D6` — the right blue family but brighter/more saturated than the true muted `#044ab3` → OKLab **ΔE 0.0592**. Crucially it *estimated from pixels*: it proposed a **different** hex than the truth (the opposite of Carbon's exact-hex recall) even while reading the on-screen "Cantor8" wordmark — the model had no memorized palette to fall back on. This is the honest, uncontrolled fidelity number (single read; vision hex estimates vary run-to-run).
   **Label — FINAL: "agent-proposed, human-approved."** The clean read lands ΔE 0.0592, essentially at the 0.05 "small perceptual delta" bar: the agent derives the correct accent *family* and the human gate finalizes the exact value — precisely the "agent proposes, human decides" flow this layer is built on. Recorded honestly, derivation is a strong *starting point* (right family, a small human correction), **not** a pixel-perfect match. The three data points: Verdant 0.05 (closed loop), Carbon 0 (recall — confounded), Cantor8 **0.0592 (clean, uncontrolled)**. On this, the derivation trace takes headline billing on the private instance (decision rule); the round-trip diff ships as the public demo.
