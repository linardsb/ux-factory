...
1. **Derivation fidelity** *(gates the headline claim's labeling)*
   ...
   **Outcome (2026-07-19) — PROVISIONAL, pending the real-product half.** The Verdant round-trip ran the full capability end to end: recorded vision run → proposed seed → human gate → deterministic fidelity diff (trace `traces/pack-seed-verdant.jsonl`; diff `tooling/round-trip/verdant.diff.json`; ticket #40). The agent proposed `color-accent #2d6a48` against ground truth `#2f7a4d` → OKLab **ΔE 0.05** — right at the ≤0.05 threshold (an honest slightly-dark read, not a rigged near-zero); the type ramp, spacing (4px grid), and radius are all usable; the proposed palette passes WCAG AA on 12/12 ruleset pairs. On the decision rule the **proposal clears the "agent-proposed, human-approved" tier**. The human gate approved the seed and corrected the single token it read dark (`color-accent → #2f7a4d`), recorded in the seed's `review.corrections` — the visible "agent proposes, human decides" boundary; the fidelity diff measures the agent's *raw* proposal (ΔE 0.05), never the corrected value.
   **This label is provisional.** The rule keys on the Verdant round-trip **and** an offline real-product test; only the former has run. Verdant is a CLOSED round-trip against the engine's own `derive()` output — a controlled, favourable case (reading a solid accent off a clean render is near colour-picking), so a near-threshold ΔE is expected and proves the PIPELINE, not high-fidelity derivation on messy real input. The final label rests on the offline real-product test (real screenshots + published tokens, real ambiguity), run locally with nothing real-brand committed (privacy boundary, hard). Follow-up open below; fold its numbers here to finalize. Until then the capability is labelled **"agent-proposed, human-approved (provisional — Verdant round-trip only)."**
...
## Open questions
- [ ] **Spike 1 real-product half — blocks finalizing the derivation label.** Run the capability offline on one real product with a published design system (screenshots + published tokens; local, nothing real-brand committed — privacy boundary); record the accent ΔE + scale-usability numbers in §Spikes 1 and resolve the provisional "agent-proposed, human-approved" label. (#40 landed the Verdant round-trip + the provisional label; this is the remaining conjunct of the decision rule.)
