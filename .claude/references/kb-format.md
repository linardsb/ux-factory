...
- `system/specs/<component>.md` — head schema v1:
  ...
  - `props` — `{ <name>: { type, required, enum?, description?, min?, max?, step? } }` — names, types, requiredness, guidance: exactly what the vocabulary generator needs. `min`/`max`/`step` are optional **numeric-control bounds** for the live playground (#211) and are valid **only** on a `type: "number"` prop — a bound on a string or boolean prop is a parse error, not a silent no-op. They ride into `vocabulary.json` verbatim, unlike `example`: a bound is a *constraint*, which makes an agent's output more bounded, never more copied.
  ...
  - `example` (optional) — a props object that is the component's **live playground starting state**, authored beside the component it documents (#211). Two validations, deliberately split: `parseComponentSpec` checks only that it is an object, and the **semantic** check is `validateExamples` in `agent-layer/gen-vocabulary.mjs`, which runs the shipped `validateComposition` over `{ name, props: example }` against the fully built vocabulary — so an example that would not render is a **red build naming the spec path**, never a broken playground a visitor discovers. It is **not** projected into `vocabulary.json`: that file is a *prompt input* for `record-composition.mjs`/`record-build.mjs`, both of which are fenced against being fed an example. `example` lives on the pack side (`...s.head` → `pack.json`), which is what the docs read and no recorder does.
...
Sync rule: the physical shape (leading ```json fence + `## ` sections) must stay readable by `portal/lib/kb.mjs`'s `parseFencedJson`/`section` — change the shape, keep both parsers in sync. Adding an *optional* head key (like `aiPatterns`, or #211's `example`) does not change the shape — the fence still parses as one JSON object and the prose sections are untouched — so `parseFencedJson`/`section` stay compatible and no portal-side change is needed.
