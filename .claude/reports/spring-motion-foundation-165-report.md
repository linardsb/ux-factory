...
- Task 4: tokenize + spring the touched literals → `system/proto.css` (UPDATE: `.ot-notes`, `.ot-btn`, `.ot-switch::after`, `.ot-sheet`, `.ot-toast`; pure color/border lines left as-is). `.ot-btn`'s press going `0.08s ease` → `var(--motion-fast)` (160ms) + bounce is intended: the contract has no sub-160ms duration token, so `--motion-fast` IS the nearest-token substitution, and the springier release is the ticket's pairing rule for a thing you touch (review #179 Low 4 confirmed)
