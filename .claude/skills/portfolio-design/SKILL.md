...
Direction layer adapted from Anthropic's `frontend-design` skill (anthropics/claude-code plugin). Numeric craft layer synthesized from Dammyjay93/interface-design (type ratios, 60/30/10, easing discipline) — rated the strongest community craft skill 2026-07. Correctness layer synthesized from vercel-labs/web-interface-guidelines (MUST/SHOULD/NEVER a11y + motion correctness; Web-Interface-Guidelines lineage). Rules are re-expressed for this repo's vanilla + token constraints, not copied. The refusal list in CHECKLIST.md and the four network states in CRAFT.md come from the 2026-09-15 borrowability audit (#421): Impeccable's `craft-floor.md` (Apache-2.0), taste-skill §9.F (MIT) and Tranz007's `state-sweep` (MIT), each rule marked with its source in a trailing parenthesis, and the two rules the audit rejected written down beside where they would sit.
