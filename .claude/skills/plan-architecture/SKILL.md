...
**Input intent**: $ARGUMENTS — a PRD, an epic, a brief, or a free-form idea. If it's a path to a PRD/epic doc
(e.g. `docs/epics/<slug>.prd.md` from `plan-create-prd`), **read it first** — it's the intent this spec serves.

**Reference docs (optional):** if any paths were passed alongside the intent — API docs, product/engineering
docs, ADRs, prior research, a competitor teardown — **read them first.** They ground the exploration so you
propose options that fit what already exists instead of inventing. If none were passed, **ask whether any exist**
before you start exploring — a lot of the context you need is usually already written down.
...
- **System behaviour (systems check)** — the loops the design creates or touches: what accumulates (queues,
  costs, state, tech debt), which feedback arrives delayed (the failure you only meet weeks later), where the
  bottleneck sits once it works, and how any metric or incentive in it could be gamed. Naming these now is
  cheaper than meeting them in production.
...
Recommend a direction for each, with the reasoning, and let the user make the call. Present the competing
approaches — and the data-model shape — **visually** before asking for the call (a small Mermaid or component
sketch per option, per the `show-me` skill's conventions): the user should compare shapes, not paragraphs. Skip
what doesn't apply — and say so, don't silently omit it.
...
Only after the calls are made. Pick where it lives. **The strong default is a separate markdown doc in the repo,
linked both ways to the PRD/epic:** the PRD stays pure intent, the architecture (the *how*) lives in its own
decision doc beside it, and each links to the other. Keeping them as two clean, linked sources is what lets
`piv-slice-epic` and `piv-plan-implementation` read intent and architecture separately later. The options:
...
- **A separate linked doc in the repo** *(recommended)*: write `docs/epics/<slug>.architecture.md`, add a link back
  ...
  replace `architecture: TBD` in the PRD's status header with `architecture: decided <date>`. Two clean, linked
  sources.
...
In either PRD-linked option, if the PRD carries no status header — every PRD written before that rule — add one per
`plan-create-prd` before writing the date, rather than skipping the rung: there is no `architecture: TBD` to replace.
...
Intent: [<slug>.prd.md](./<slug>.prd.md)   ← link back to the PRD this spec serves
...
Confirm where you wrote it, that you linked it both ways to the PRD, summarize the recommended approach + the key
calls in a few lines, then offer the natural next moves and let the user pick — **don't force a pipeline**:

- **Slice it into tickets** — feed the PRD **and** this architecture doc to `/piv-slice-epic` to break the epic into
  PIV-sized tickets and create them as GitHub Issues.
- **Keep going here** — stay in this conversation to refine the decisions, or to create the issues directly.
...
- ✅ **Separate architecture doc linked both ways to the PRD** (intent and approach stay two clean sources).
