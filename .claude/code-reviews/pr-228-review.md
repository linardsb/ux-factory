...
| **M5** 18×18 handle | **fixed** | `.stx-grab` is 24×24 (SC 2.5.8), and studio.css's header now records the criterion and states that the Spacing exception was *not* relied on. No group 12 pin covers `.stx-grab`; studio.html is not in the VR page set, so no baseline cascade — which also means **nothing gates its appearance**, so the handle was screenshot at 4× against the 18px original: growing the target alone turned the repeating dot field into a texture square, so the grip is painted at 12×12 inside the 24px target (`padding` + `background-origin/clip: content-box`). `elementFromPoint` at the handle's centre returns `.stx-grab` over all 10 whole-card links it overlaps |
