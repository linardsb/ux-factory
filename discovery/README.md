...
{ "slug": "faster-payment-run-1", "provenance": "fictional", "label": "Real run — fictional scenario",
  ...
  "turnStats": [ { "turn": "t1", "numTurns": 2, "durationMs": 21187, "costUsd": 0.043, "ok": true, "ts": "…Z" } ] }
```
...
```bash
node tooling/build-checks.mjs                      # group 29 drives every applier rule, group 30 the session — SDK-free
cd portal && node lib/discovery-transport.mjs --preflight   # the transport's seven rows, zero tokens, before a real run
