...
- `depth` and `branch` take the values `bank.mjs` exports (#282 owns the names; #285 owns the
  selection); `entryMode` is #286's. The values shown are today's names, not a contract. #285
  replaces `branch` with `facets`, the five-key boolean vector whose keys are `bank.mjs`'s `FACETS`;
  packages recorded before it carry no `facets` field and read as the unfaceted list.
