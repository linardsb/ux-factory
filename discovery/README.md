...
discovery/
  ...
  <slug>/                one run package, fictional runs only (real runs live in the jobs folder)
    ...
    build/               the DESIGN half, written by the canvas and by nothing else: ops.jsonl (the
                         truth, append-only) + canvas.json (the arrangement, derived). Absent on a
                         package that has never been on a canvas (§The build half, #302)
...
## The build half (`build/`, #302)

`<slug>/build/` is where a package's DESIGN lives, beside the decisions it came from. It is written
by the canvas and by nothing else — not by a session, not by a proposal run — so the counts above do
not move: a run package is still three files during a session and five after a proposal run, and
`build/` is a folder that may or may not sit next to them. A package that has never been on a canvas
has no `build/` at all, and `loadBuild` returns **null** for it rather than throwing, because asking
...
One line per op, append-only, in the shape the architecture pins:

```jsonl
{ "seq": 1, "at": "…Z", "source": "owner", "op": "screen.compose", "params": { "screenId": "add-payee", "why": "…", "composition": { … }, "decisionRefs": ["7", "8"] }, "status": "applied" }
{ "seq": 2, "at": "…Z", "source": "owner", "op": "frame.size", "params": { "frameId": "f1", "preset": "phone" }, "status": "applied" }
{ "seq": 6, "at": "…Z", "source": "owner", "op": "connect", "params": { "from": { "frameId": "f1", "partId": "continue" }, "to": { "frameId": "f2" }, "trigger": "…" }, "status": "applied" }
```
...
**JSON Canvas–SHAPED, and deliberately NOT JSON Canvas 1.0** — settled as **D-b** (owner,
2026-09-18): ship the dialect, and make the file say so in its own `$description` rather than leave a
reader to hit it. Four divergences, each asserted BY NAME by group 36, because the failure mode is a
...
`ops.jsonl` is appended line by line (a ledger rewritten whole is a ledger a crash can lose the
middle of) and `canvas.json` is a whole-file rewrite.

### How the committed spine was produced

`discovery/faster-payment/build/` is MVP 14's spine and the first committed artifact in this repo
whose subject is a DESIGN rather than a decision or a run. It is the package's own argument, not a
demo: decision 7 says the customer has to finish this without ringing us and decision 8 says the
riskiest assumption is that a screen can do the fraud check the phone agent used to do — so an
add-payee screen that asks for the account number alone and commits with one button, and an error
state that is the Confirmation of Payee non-match. The compose op's `why` says that and names both
decisions in `decisionRefs`.

**Never hand-written, and that is D-a (owner, 2026-09-18).** `ops.jsonl` is the truth, so it may not
be hand-authored as a *recording*. But these are the owner's ops, and an owner op typed by the owner
is what the op grammar exists to accept. So the six ops were **typed**, applied through the **real
`applyOps`**, and written by the **real `canvas-store.saveBuild`** — the `seq`, the ids and the
applied document all come from the applier, or the gate's fixture is fiction. To reproduce it: type
the ops, fold them with `applyOps` from `system/canvas-ops.mjs`, and hand the resulting document and
the op lines to `saveBuild`. No committed CLI, and no editor.
