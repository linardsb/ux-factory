...
- **New discovery op verb or run** → a verb is a `discovery/ops.mjs` edit (`OPS`, its `PARAMS` entry, the switch case and its build-checks group 29 fixture, together, under the epic's op-verb lock — no other ticket adds one concurrently). A run is a REAL session (the recorder is #284): `answers.jsonl` is server-written and verbatim, `transcript.jsonl` is append-only, and neither is ever hand-written or hand-edited — a bad run is re-run. Provenance decides the root: fictional → `discovery/<slug>/`; real → `<JOBS_DIR>/_discovery/<slug>/`, never committed. Format → `discovery/README.md`.
