...
tooling/
  build-checks.mjs            38 PURE groups, in CI — the repo's main gate  (→ references/gates.md)
...
- **New discovery op verb or run** → a verb is a `discovery/ops.mjs` edit (`OPS`, its `PARAMS` entry, the switch case, its build-checks group 29 `VALID_FOR` fixture and a group 31 section home — 31.2 iterates the verbs against the projection's `SECTIONS` rows, so a verb with no home fails by name — together, under the epic's op-verb lock — no other ticket adds one concurrently). A run is a REAL session through the portal's discovery drawer (#284) — a blank idea under Think, Create PRD or Grill, or an existing PRD audited under Grill (#286), whose document is the one server-written `kind: "document"` answer line: `answers.jsonl` is server-written and verbatim, `transcript.jsonl` is append-only, and neither is ever hand-written or hand-edited — a bad run is re-run. Provenance decides the root: fictional → `discovery/<slug>/`; real → `<JOBS_DIR>/_discovery/<slug>/`, never committed. **A proposal is not an op** and has no route into the op grammar — see the proposal-run bullet below. **Park, look-it-up and the escape hatch are not ops either** (#289): they are three drawer CONTROLS that file through the four existing verbs and add none, and the person's declared `intent` on an off-script answer line is server-written like every other field. Format → `discovery/README.md`.
- **New discovery proposal run** → a REAL run through the portal's discovery drawer ("Propose features", #359), over a package whose session is **finished** — the route refuses an open one by name, and a package gets **ONE** proposal run with no override. `proposals.jsonl` is server-written and append-only; `proposals.md` is regenerated on every verdict. **Never hand-written, never hand-edited** — a bad run is fixed by tightening ONE prompt constant in `portal/lib/discovery-proposer.mjs`, DISCARDING `proposals.jsonl` and re-running, at most three paid attempts, `--dry` before each. A proposal is **not** a decision and has no route into the requirement hierarchy: the owner's verdict records that they liked it and nothing more. Format → `discovery/README.md` §Feature proposals.
...
- **Testing:** no suite, no linter, no type-check — don't hunt for or invent one. **The one exception is CI's CodeQL gate (#387), which blocks a PR on any high or critical alert that the PR's OWN DIFF raises AND on any that `main` already carries (#408) — two legs, because the merge-ref read is diff-scoped on pull requests and would otherwise let an alert nobody touches sit open forever (#400). So an inherited alert reds every open PR at once and the fix belongs on `main`, not in whichever PR ran next. It is a gate, not a suite; the threshold and both scopes are settled — leave it.** "Done" = run the surface you touched: portal boots and `/api/health` answers; a generator prints its `✓` line; a page renders under the neutral pack. Which gate proves what → `.claude/references/gates.md`.
...
- **`gates.md`** — the gate stack: build-checks' 38 groups, the five journey drivers, the pixel gate, the morph gates, and what each one states it CANNOT reach. Read before adding or changing a gate, or before trusting a green run.
