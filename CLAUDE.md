...
portal/                     local-first workbench (127.0.0.1 only, never deployed)
  ...
  record-composition.mjs    build-time composition runner (CLI, not deployed): a REAL Agent SDK run composes a {name,props,children} dashboard view for one Fieldwork question → proto/compositions/<slug>.json + a PIV trace pair; prompt built ONLY from vocabulary + fixtures + question + slot bounds (no example); reuses recordRun + validateComposition + curateTrace; keep-gate is validateTrace (epic #1 ticket #13, folds spike 6)
...
- **New composition proposal** → a REAL run: `node portal/record-composition.mjs "<question>" <summary-strip|insight-panel>` (`--dry` first to prove auth). It writes `proto/compositions/<slug>.json` + a validating PIV trace pair and upserts `index.json`; verify the numbers against `node tooling/fieldwork-kpis.mjs`. Same honesty rule as traces — never hand-write a composition or hand-feed an example; a weak/invalid run is fixed by tightening the runner's committed prompt + re-run.
