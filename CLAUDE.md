...
- **Testing:** no suite, no linter, no type-check — don't hunt for or invent one. **The one exception is CI's CodeQL gate (#387), which blocks a PR on any high or critical alert that the PR's OWN DIFF raises — it is diff-scoped on pull requests, so an alert sitting open on `main` that this PR does not touch blocks nothing (#400). It is a gate, not a suite; the threshold and the scope are both settled — leave it.** "Done" = run the surface you touched: portal boots and `/api/health` answers; a generator prints its `✓` line; a page renders under the neutral pack. Which gate proves what → `.claude/references/gates.md`.
