...
- **Testing:** no suite, no linter, no type-check — don't hunt for or invent one. **The one exception is CI's CodeQL gate (#387), which blocks a PR on ANY open high or critical alert on its merge ref — not only a new one, because there is no baseline delta: it is a gate, not a suite, and it is deliberate — leave it.** "Done" = run the surface you touched: portal boots and `/api/health` answers; a generator prints its `✓` line; a page renders under the neutral pack. Which gate proves what → `.claude/references/gates.md`.
