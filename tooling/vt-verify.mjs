...
console.log(failed
  ...
  : `\nvt-verify ✓  morphs real · load accounted for · renames instant · reduced motion off — /build + ${SITEWIDE.length} site-wide surfaces + the studio canvas, whose zoom, placement and #205 move verbs all name nothing, /factory's #209 replay driver, sampled DURING its fourteen-second playback as well as after it, its #207 compile beat, whose crossfade opens no transition at all — that one under reduced motion too — and (#213) its take-over handover and #210 keep-rail copy + export clicks plus (#214) the method-card redraft and (#217) the marquee, the group move and the context menu — the interactions the earlier samples predate — sampled the same way with movement proven first, full motion only by design, in every one (${toRun.join(", ")})`);
