...
async function selectPass(browser, engineName, t, errors) {
  ...
  const openAt = async (x, { stripFlip = false, w = NODE_W } = {}) => {
    ...
    return p7.evaluate(async ([id, strip]) => {
      ...
      return {
        ...
        anchor: Math.round(sr.left + parseFloat(menu.style.getPropertyValue("--x") || 0)),
  ...
  // THE CSS CHANGED WITH THE SUBSTRATE and the claim follows it. `justify-self: end` was a GRID
  // property and there is no grid; the rule is `translate: -100% 0`, so a flipped menu's RIGHT
  // edge lands ON its anchor and it opens leftwards INTO the stage instead of growing off it.
  // THE ANCHOR IS setPos'S ANSWER, NOT menuAnchor'S ASK, and the difference is worth naming rather
  // than asserting past. menuAnchor says 2792 and flips, then setPos clamps the menu to
  // STAGE_W - MENU_W = 2596 like every other node — so the menu's --x is the clamped value and the
  // flip translates from there. Asserted against what the menu actually carries, so this row states
  // the shipped geometry instead of an expectation the clamp makes unreachable.
  //
  // WHAT THIS LEAVES, recorded because no gate says it: within a menu's width of the right edge the
  // menu is clamped AND flipped, so it renders up to MENU_W left of the component it belongs to.
  // The clamp alone would keep it on the stage. Which of the two should win is a design call, not a
  // coordinate migration, so it is reported rather than decided here.
  t(`#217 · R5 — a menu opened on a FAR-EDGE (x ${FAR_X}) component sets data-flip-x, and the CSS rule is LIVE: its right edge lands on the anchor setPos gave it`,
    Boolean(flipped) && flipped.flipX === true
    && Math.abs(flipped.right - flipped.anchor) <= 1, JSON.stringify(flipped));
  // THE MUTATION that decides whether the row above can fail: same menu, same place, attribute
  // removed. If .stx-menu[data-flip-x] { translate: -100% 0 } ever stopped being in the sheet, the
  // two measurements would be identical and this goes red. The DIRECTION is the opposite of the
  // grid rule's: stripping the flip moves the box RIGHT by exactly its own width.
  const unflipped = await openAt(FAR_X, { stripFlip: true, w: MIN_SIZE });
  t("#217 · R5 · THE MUTATION — removing data-flip-x really moves the box, by exactly its own width, so the rule is wired rather than merely written",
    Math.abs((unflipped.left - flipped.left) - flipped.width) <= 1 && unflipped.width === flipped.width,
  ...
  t("#217 · …while an INTERIOR component's menu carries no flip at all, so the attribute is proven conditional rather than always on",
    interior.flipX === false && interior.x === `${INTERIOR_X}px`, JSON.stringify(interior));
  // The clamp is what makes the two rows above read differently, so it is stated as its own fact:
  // an interior anchor is written through unchanged, a far-edge one is not.
  t("#217 · …and an interior anchor reaches the menu UNCLAMPED while the far-edge one is clamped to STAGE_W - MENU_W — the measurement the row above reports rather than an excuse",
    interior.x === `${INTERIOR_X}px` && flipped.x === `${STAGE_W - NODE_W}px`,
    `interior ${interior.x}, far edge ${flipped.x}, the clamp is ${STAGE_W - NODE_W}px (MENU_W is NODE_W)`);
