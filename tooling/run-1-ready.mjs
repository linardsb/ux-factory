...
//   1  the one-sentence input exists and is TRACKED                          (precondition 1)
//   2  the sealed pre-registration exists, is TRACKED and is non-empty       (precondition 2)
...
// Tracked, not merely present: git history is the receipt that a file predates the run, and an
// untracked file has no history to be earlier than anything.
function tracked(p) {
  try {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const r = runOneReady();
    console.log(`run-1 ready ✓  ${r.checks} checks · ${r.preset} preset composes ${r.questions} · sealed committed ${r.sealedCommittedAt ?? '(no commit yet)'}`);
  } catch (e) {
