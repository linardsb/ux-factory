...
// this group's temp dir, under real provenance, so its package lands under TMP and nowhere else (#383).
{
  ...
  // package proven to carry NO created key, so the GET, turn and close routes never claim a create; and
...
console.log(JSON.stringify([row(openSession(audit)), row(openSession(audit)), row(openSession(blank))]));`;
    ...
    ok(!Object.hasOwn(sessionView(auditRoot), "created"), "case 44: sessionView carries `created` — only openSession knows which branch ran, and the GET, turn and close routes must return the plain view");
