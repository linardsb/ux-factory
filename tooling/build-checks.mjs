...
// A dropped guard (a red run) could write the reserved slug; the case removes exactly that path after.
{
  ...
  // removes it, and a denial is RECORDED only while the set is empty — DENIED either way. Driven
  ...
  // was red at "inside the bracket" (the #343 report has the run).
  {
    ...
    ok(denied(await fire("PreToolUse", { tool_name: "Bash", tool_input: { command: "find ." }, tool_use_id: "u4" })) && onDisk().length === 1,
    ...
    ok(denied(r5) && onDisk().length === 2 && onDisk()[1].tool === "Read" && heard.length === 2, `case 20: after the last SubagentStop a denial must be recorded again (disk ${onDisk().length}, heard ${heard.length})`);
    ...
    ok(same(r6, { continue: true }) && onDisk().length === 3 && onDisk()[2].tool === toolNameFor("record_decision") && onDisk()[2].error === "answer_ref a9 does not resolve",
    ...
    ok(onDisk().length === 3, `case 20: an allowed call must record nothing (disk ${onDisk().length})`);
    ...
    ok(denied(await fire("PreToolUse", { tool_name: "Bash", tool_input: { command: "pwd" }, tool_use_id: "u8" })) && onDisk().length === 4,
