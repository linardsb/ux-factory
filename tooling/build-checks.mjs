...
// A dropped guard (a red run) could write the reserved slug; the case removes exactly that path after.
{
  ...
  // no flag_weak_answer, so the route that would fold the branch into all four stamps — 142 paid turns
  // across instrument-loans-1, graded-think-a and graded-opus-a — fails here by name, with the bill in
  // the message. The other route into that branch is a key in FINGERPRINT_INPUTS_FOR, pinned by case 30.
  {
