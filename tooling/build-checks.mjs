...
// this group's temp dir, under real provenance, so its package lands under TMP and nowhere else (#383).
{
  ...
  // one-input-set join is proven UNCHANGED and Think's stamp pinned to the five recordings' literal —
  ...
  // edit re-records those fixtures and re-pins this line in the same PR; anything else is drift.
  {
    ...
    ok(POSTURES.think.fingerprint === "7efdde37441fbd2591ba4a7dfeecdb6b" && POSTURES["think-opus"].fingerprint === "cadb38117a2660c036d87e32323a8745", `case 30: Think's stamps are ${POSTURES.think.fingerprint.slice(0, 8)} / ${POSTURES["think-opus"].fingerprint.slice(0, 8)} — the five recordings carry 7efdde37 / cadb3811. A deliberate Think edit re-records them and re-pins this line in the same PR; anything else is drift`);
...
console.log(JSON.stringify([row(openSession(audit)), row(openSession(audit)), row(openSession(blank))]));`;
  ...
  // 33.15 the two graded). The message below derives the list, so it can only ever name the truth.
  {
    ...
    ok(carriers("7efdde37441fbd2591ba4a7dfeecdb6b").length + carriers("cadb38117a2660c036d87e32323a8745").length === 5,
      `30.46: ${carriers("7efdde37441fbd2591ba4a7dfeecdb6b").length + carriers("cadb38117a2660c036d87e32323a8745").length} recording(s) carry Think's two stamps, not the five discovery-postures.mjs's header states — think: ${carriers("7efdde37441fbd2591ba4a7dfeecdb6b").join(", ") || "none"} · think-opus: ${carriers("cadb38117a2660c036d87e32323a8745").join(", ") || "none"}. Update the header in the same edit`);
