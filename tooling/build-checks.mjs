...
// A dropped guard (a red run) could write the reserved slug; the case removes exactly that path after.
{
  ...
  // ==================================================================================================
  // #289 — THE THREE AFFORDANCES. Cases 30.44-30.56.
  // ==================================================================================================

  // 30.44 — THE CASE THE WHOLE DESIGN RESTS ON. With `park` false and `affordance` null every builder
  // produces exactly the bytes it produces today, on BOTH prompts. The failure names the BUILDER, where
  // 30.45's hash pin names only "something moved" — write and run this one before anything else.
  {
    for (const [id, build] of [["think", buildThinkTurn], ["create-prd", buildCreatePrdTurn], ["grill", buildGrillTurn]]) {
      ...
      ok(a.systemPrompt === b.systemPrompt && a.prompt === b.prompt, `30.44: ${id}'s build is NOT byte-identical with the #289 parameters at their defaults — a conditional slot added a newline or an empty paragraph, which moves every stamp the recordings carry`);
    }
    ...
    ok(ga.systemPrompt === gb.systemPrompt && ga.prompt === gb.prompt, "30.44: Grill's AUDIT template is not byte-identical with the #289 defaults");
  }

  // 30.45 — TWO CLAIMS, and only one of them is "nothing moved". Think's and Think-on-Opus's stamps are
  ...
  // 33.15 the two graded). The message below derives the list, so it can only ever name the truth.
  {
    ...
    ok(POSTURES.think.fingerprint === "7efdde37441fbd2591ba4a7dfeecdb6b", `30.45: Think's prompt surface MOVED — ${POSTURES.think.fingerprint}. ${carriers("7efdde37441fbd2591ba4a7dfeecdb6b").join(", ") || "(nothing on disk carries it)"} carry the old one; a Think edit is a ticket that re-records them`);
    ok(POSTURES["think-opus"].fingerprint === "cadb38117a2660c036d87e32323a8745", `30.45: Think-on-Opus's prompt surface MOVED — ${POSTURES["think-opus"].fingerprint}. ${carriers("cadb38117a2660c036d87e32323a8745").join(", ") || "(nothing on disk carries it)"} carries the old one`);
    ...
    ok(POSTURES.grill.fingerprint === "76b7847d4ebbd9d8f16f9726ff0f4f0f", `30.45: Grill's prompt surface MOVED — ${POSTURES.grill.fingerprint}. discovery/partner-audit-1's three turns carry the old one and would go stale`);
    ...
    ok(POSTURES["create-prd"].fingerprint === "f0e7599c7bc953b74ff3750dceca5061", `30.45: Create-PRD's stamp is ${POSTURES["create-prd"].fingerprint}, not the value #289 moved it to (it was edc7c52db9d58d59213e93e65cd8d28c before DOMAIN_RULE went into its system prompt). No committed package runs create-prd, so the move costs nothing on disk — but it is a move, and it is pinned`);
    ok(DOMAIN_RULE.length > 0 && buildCreatePrdTurn(FINGERPRINT_INPUTS).systemPrompt.includes(DOMAIN_RULE), "30.45: DOMAIN_RULE is not in Create-PRD's system prompt — the one deliberate stamp move");
    for (const [id, build, inputs] of [["think", buildThinkTurn, FINGERPRINT_INPUTS], ["grill", buildGrillTurn, FINGERPRINT_INPUTS], ["grill-audit", buildGrillTurn, AUDIT_FINGERPRINT_INPUTS]])
      ok(!build(inputs).systemPrompt.includes(DOMAIN_RULE), `30.45: DOMAIN_RULE reached ${id}'s SYSTEM prompt — that moves a stamp six (Think) or three (Grill/partner-audit-1) recorded turns carry`);
    ok(same(Object.keys(FINGERPRINT_INPUTS_FOR), ["grill"]), `30.45: FINGERPRINT_INPUTS_FOR is keyed ${JSON.stringify(Object.keys(FINGERPRINT_INPUTS_FOR))} — #289's input sets must NOT be added to it, or they move every posture's own stamp`);
  }

  // 30.46 — every POSTURES entry still carries EXACTLY five keys. Restated here because #289 is the
  ...
  for (const [id, p] of Object.entries(POSTURES))
    ok(same(Object.keys(p).sort(), ["build", "fingerprint", "id", "label", "model"]), `30.46: posture "${id}" carries ${JSON.stringify(Object.keys(p))} — the affordance stamp is a MODULE CONSTANT, never a sixth key`);

  // 30.47 — TOOL_DESCRIPTIONS is still keyed exactly OPS. No WebSearch entry: one added key moves BOTH
  // shipped Think stamps (34.13's rule), and the fetch tools are not op tools.
  ok(same(Object.keys(TOOL_DESCRIPTIONS).sort(), [...DISCOVERY_OPS].sort()), `30.47: TOOL_DESCRIPTIONS is keyed ${JSON.stringify(Object.keys(TOOL_DESCRIPTIONS))}, not exactly OPS — every key is inside every posture's hash`);

  // 30.48 — AFFORDANCE_FINGERPRINT: keyed by POSTURES' own keys, 32-hex, deterministic, MOVED by one
  ...
  {
    ok(same(Object.keys(AFFORDANCE_FINGERPRINT), Object.keys(POSTURES)), `30.48: AFFORDANCE_FINGERPRINT is keyed ${JSON.stringify(Object.keys(AFFORDANCE_FINGERPRINT))}, not by POSTURES' own keys — a fifth posture must appear here as a missing key rather than as silence`);
    for (const [id, hex] of Object.entries(AFFORDANCE_FINGERPRINT)) {
      ok(/^[0-9a-f]{32}$/.test(hex), `30.48: ${id}'s affordance stamp is ${JSON.stringify(hex)}, not a 32-hex md5`);
      ok(hex !== POSTURES[id].fingerprint, `30.48: ${id}'s affordance stamp EQUALS its posture stamp — then it covers nothing the posture stamp did not`);
      ok(affordanceFingerprintOf(POSTURES[id]) === hex, `30.48: ${id}'s stamp is not reproducible from its own posture — the transport computes it off the RESOLVED posture and must land on the same value`);
    }
    ok(AFFORDANCE_FINGERPRINT.think !== AFFORDANCE_FINGERPRINT["create-prd"], "30.48: think's and create-prd's affordance stamps are equal — the off-script TURN body is shared but the SYSTEM prompt is the posture's own, and DOMAIN_RULE sits in one of them, so one hash wearing four keys would cover neither");
    ...
    ok(affordanceFingerprintOf(resolvePosture({ posture: "grill", model: "claude-opus-5" })) !== AFFORDANCE_FINGERPRINT.grill,
      "30.48: a Grill-on-Opus run's affordance stamp equals the sonnet one — a by-id lookup would stamp a surface the turn did not run under");
    ...
    ok(AFFORDANCE_INPUT_SETS.length === 3, `30.48: the affordance stamp covers ${AFFORDANCE_INPUT_SETS.length} input set(s) — park, aside and look-up are three different prompts`);
    ok(buildThinkTurn(AFFORDANCE_FINGERPRINT_INPUTS).prompt !== buildThinkTurn(LOOKUP_FINGERPRINT_INPUTS).prompt, "30.48: the aside and look-up input sets build the SAME prompt — one of the two rule strings is then outside every stamp");
    ok(buildThinkTurn(PARK_FINGERPRINT_INPUTS).prompt !== buildThinkTurn(FINGERPRINT_INPUTS).prompt, "30.48: the park input set builds the ordinary turn prompt — PARK_RULE is then outside every stamp");
    ...
    // rather than by editing the module, the way case 19 moves the posture stamp.
    {
      ...
      ok(affordanceFingerprintOf({ build: spaced, model: POSTURES.think.model }) !== AFFORDANCE_FINGERPRINT.think, "30.48: one trailing space on the affordance body did not move the stamp — the hash is not over the prompt");
    }
    for (const set of AFFORDANCE_INPUT_SETS) {
      ok(Object.isFrozen(set) && Object.isFrozen(set.answers) && set.answers.every(Object.isFrozen), "30.48: an affordance input set is not frozen at every level — an input set is a permanent committed literal");
      ok(set.answers.every((a) => typeof a.text === "string" && a.text.startsWith("A fixed ")), `30.48: an affordance input set carries text that is not the synthetic literal — ${JSON.stringify(set.answers.map((a) => a.text))}. FINGERPRINT_INPUTS.answer.text is "A fixed answer." for exactly this reason: a set seeded from a real answer bakes a person's words into the module forever`);
    }
    ...
    ok(buildThinkTurn(AFFORDANCE_FINGERPRINT_INPUTS).prompt.includes(pendingBrief(AFFORDANCE_FINGERPRINT_INPUTS.answers, AFFORDANCE_FINGERPRINT_INPUTS.ledger)),
      "30.48: the aside input set's build does not carry pendingBrief's output — then pendingBrief is outside the affordance stamp");
    ok(pendingBrief(AFFORDANCE_FINGERPRINT_INPUTS.answers, AFFORDANCE_FINGERPRINT_INPUTS.ledger).length > 0, "30.48: the aside input set's brief is empty — the fixture's aside must be UNFILED against FINGERPRINT_INPUTS' ledger, or the coverage above is vacuous");
  }

  // 30.49 — the four rule constants, pinned VERBATIM (case 16's idiom), each present in the build that
  // must carry it and ABSENT from every build that must not.
  {
    ...
    for (const [name, text] of Object.entries(RULES))
      ok(live[name] === text, `30.49: ${name} is not the pinned text — a tightening is a one-line diff this case notices, and each of these is prompt text a real run reads:\n      ${JSON.stringify(live[name]?.slice(0, 120))}`);
    ...
    for (const [id, build] of Object.entries(POSTURES).map(([k, p]) => [k, p.build])) {
      ...
      ok(park.includes(PARK_RULE), `30.49: ${id}'s PARK turn prompt does not carry PARK_RULE`);
      ok(aside.includes(ESCAPE_HATCH_RULE) && !aside.includes(LOOK_IT_UP_RULE), `30.49: ${id}'s ASIDE turn prompt carries the wrong intent's rule`);
      ok(look.includes(LOOK_IT_UP_RULE) && !look.includes(ESCAPE_HATCH_RULE), `30.49: ${id}'s LOOK-UP turn prompt carries the wrong intent's rule`);
      ok(aside.includes(DOMAIN_RULE) && look.includes(DOMAIN_RULE), `30.49: ${id}'s off-script turn prompt does not carry DOMAIN_RULE — MVP 10's domain rule reaches Think and Grill by this route ALONE`);
      // ABSENT from an ordinary banked build, both prompts. This is what 30.44's byte-identity means in
      // words, and it fails naming the string rather than the hash.
      for (const rule of [PARK_RULE, LOOK_IT_UP_RULE, ESCAPE_HATCH_RULE])
        ok(!plain.prompt.includes(rule) && !plain.systemPrompt.includes(rule), `30.49: an affordance rule reached ${id}'s ordinary banked build`);
      ...
      ok(!aside.includes(FINGERPRINT_INPUTS.question.weakAnswer) && !look.includes(FINGERPRINT_INPUTS.question.weakAnswer), `30.49: ${id}'s off-script turn prompt carries the weak-answer note — there is no answer to judge on that turn`);
      ok(aside.includes(FINGERPRINT_INPUTS.question.id), `30.49: ${id}'s off-script turn prompt does not name the question still on the table`);
    }
  }

  // 30.50 — ledgerBrief's evidence line: ABSENT on FINGERPRINT_INPUTS' ledger (which is what keeps all
  // four stamps unmoved), PRESENT and naming seq · provenance · source when the ledger holds one.
  {
    ok(!ledgerBrief(FINGERPRINT_INPUTS.ledger).includes("Evidence filed in this run"), "30.50: ledgerBrief renders an evidence line over FINGERPRINT_INPUTS' ledger — the synthetic ledger holds no file_evidence, and an always-printed line moves all four stamps");
    ...
    ok(brief.includes("Evidence filed in this run") && brief.includes("seq 4") && brief.includes("secondary-source") && brief.includes("https://example.test/scheme"),
      `30.50: the evidence line does not name its seq, provenance and source — ${JSON.stringify(brief.slice(-200))}`);
    ok(ledgerBrief(withEv).indexOf("Evidence filed") > ledgerBrief(withEv).indexOf("Parent candidates"), "30.50: the evidence line must come AFTER the parent-candidates block — PARENT_RULE's recency tail is what #341 paid a recording for");
  }

  // 30.51 — the builders' refusal battery, each named in its message.
  for (const [id, build] of Object.entries(POSTURES).map(([k, p]) => [k, p.build])) {
    ok(names(() => build({ ...FINGERPRINT_INPUTS, park: true, affordance: "aside" }), "park", "aside") === null, `30.51: ${id} accepted park AND affordance — ${names(() => build({ ...FINGERPRINT_INPUTS, park: true, affordance: "aside" }), "park", "aside")}`);
    ok(names(() => build({ ...FINGERPRINT_INPUTS, affordance: "shout" }), "shout", ...AFFORDANCES) === null, `30.51: ${id} accepted an unknown affordance — ${names(() => build({ ...FINGERPRINT_INPUTS, affordance: "shout" }), "shout", ...AFFORDANCES)}`);
    ok(names(() => build({ ...FINGERPRINT_INPUTS, park: "yes" }), "park") === null, `30.51: ${id} accepted a non-boolean park`);
  }
  ok(names(() => buildGrillTurn({ ...AUDIT_FINGERPRINT_INPUTS, affordance: "aside" }), "existing-prd", "aside") === null, `30.51: Grill accepted an off-script AUDIT turn — ${names(() => buildGrillTurn({ ...AUDIT_FINGERPRINT_INPUTS, affordance: "aside" }), "existing-prd", "aside")}`);
  ok(names(() => buildGrillTurn({ ...AUDIT_FINGERPRINT_INPUTS, park: true }), "existing-prd", "park") === null, `30.51: Grill accepted a PARK on an audit turn — the document is the answer to every question, and "absent" is the audit's own open_question verdict`);

  // 30.52 — the two session guards, BOTH directions over every branch. Pure over their arguments, so
  // this group drives them with no SDK.
  {
    ...
    ok(threw(() => assertAffordance(head(), cur(), "aside")) === null, "30.52: assertAffordance refused an open blank-idea session — the accepting control");
    ok(threw(() => assertAffordance(head(), cur(), "look-up")) === null, "30.52: assertAffordance refused a look-up on an open session");
    ok(threw(() => assertParkable(head(), cur())) === null, "30.52: assertParkable refused an open blank-idea session — the accepting control");
    ...
    ]) ok(names(fn, ...needles) === null, `30.52: ${label} — ${names(fn, ...needles)}`);
    ok(same([...AFFORDANCES], ["look-up", "aside"]), `30.52: AFFORDANCES is ${JSON.stringify(AFFORDANCES)} — two controls, never one classifier, so MVP 7 and MVP 9 pin separately in the record`);
    ok(Object.isFrozen(AFFORDANCES), "30.52: AFFORDANCES is not frozen");
  }

  // 30.53 — AC #5. READ_TOOLS is STILL exactly Read · Grep · Glob, and the fetch tools are outside it:
  // a path rule cannot reach a URL, so putting them there would make #287's assertion false.
  {
    ok(same(Object.keys(READ_TOOLS).sort(), ["Glob", "Grep", "Read"]), `30.53: READ_TOOLS is keyed ${JSON.stringify(Object.keys(READ_TOOLS))} — the path allow-list's whole reach, and #289 must not widen it`);
    ok(same([...FETCH_TOOLS], ["WebSearch", "WebFetch"]) && Object.isFrozen(FETCH_TOOLS), `30.53: FETCH_TOOLS is ${JSON.stringify(FETCH_TOOLS)}`);
    ok(FETCH_TOOLS.every((t) => !Object.hasOwn(READ_TOOLS, t)), "30.53: a fetch tool is in READ_TOOLS — then a path rule decides it, and AC #5 is false");
    const set = allowSetFor({ root: ROOT });
    for (const t of FETCH_TOOLS) {
      ok(allowsPath(set, "anything").allow !== undefined, "30.53: allowsPath is unreachable");
      ok(fenceDecision(set, t, { url: "https://example.test" }, []).allow === false, `30.53: ${t} was allowed on a BANKED turn (extraTools []) — the fetch fence opens per call, never by default`);
      ok(fenceDecision(set, t, { url: "https://example.test" }, [...FETCH_TOOLS]).allow === true, `30.53: ${t} was denied on an OFF-SCRIPT turn (extraTools FETCH_TOOLS) — the widening is by NAME, through #359's seam`);
    }
    // The widening is a hole for exactly two names and nothing else.
    for (const t of ["Write", "Edit", "Bash", "NotebookEdit", "WebSearchX"])
      ok(fenceDecision(set, t, { file_path: join(ROOT, "x") }, [...FETCH_TOOLS]).allow === false, `30.53: ${t} was allowed under extraTools: FETCH_TOOLS — the widening names two tools, it is not a door`);
    ...
    for (const t of FETCH_TOOLS) ok(allowsToolName(t) === false, `30.53: allowsToolName("${t}") is true — the session's vocabulary is the four op verbs, and the fetch widening is per-call extraTools`);
    for (const op of DISCOVERY_OPS) ok(allowsToolName(toolNameFor(op)) === true, `30.53: allowsToolName lost ${op} — the positive control`);
  }

  // 30.54a — runTurn's ORDERING, source-pinned: every #289 guard runs BEFORE the append, because
  ...
  // themselves are driven in 30.52; this is the ordering, which only source can reach.
  {
    const src = readFileSync(join(ROOT, "portal/lib/discovery.mjs"), "utf8").replace(/\/\/[^\n]*/g, "").replace(/\/\*[\s\S]*?\*\//g, "");
    const at = src.indexOf("export async function runTurn(");
    const body = src.slice(at, src.indexOf("\n}", at));
    ok(at !== -1 && body.length > 200, "30.54a: runTurn could not be located in portal/lib/discovery.mjs");
    ...
    ok(affordanceAt !== -1 && parkAt !== -1 && appendAt !== -1, `30.54a: runTurn does not call assertAffordance, assertParkable and appendAnswer (${JSON.stringify({ affordanceAt, parkAt, appendAt })})`);
    ok(affordanceAt < appendAt && parkAt < appendAt, `30.54a: a #289 guard runs AFTER appendAnswer (assertAffordance at ${affordanceAt}, assertParkable at ${parkAt}, appendAnswer at ${appendAt}) — the LOCK → GUARDS → APPEND → RUN ordering is the whole function, and an append before a refusal leaves a permanent answer line for a turn no agent ever ran`);
    ok(/park && offScript/.test(body), "30.54a: runTurn does not refuse park together with an off-script turn by name");
    ok(/questionId: null, kind, intent, text/.test(body), "30.54a: the off-script append does not force question_id null and pass the declared intent");
  }

  // 30.55 — THE DRAWER's #289 half, source-pinned (portal.js touches the DOM at module scope, so a pin
  ...
  // the status line reading the package rather than counting stream events.
  {
    ...
    ok(turnFetches === 1, `30.55: portal.js POSTs /api/discovery/turn from ${turnFetches} places — the four controls (submit · park · look it up · ask something else) share ONE poster, because two SSE parsers drift and no gate can run either`);
    const pdAt = js.indexOf("async function postDiscoveryTurn(");
    const pdBody = js.slice(pdAt, js.indexOf("\n}", pdAt));
    ok(pdAt !== -1 && /fetch\('\/api\/discovery\/turn'/.test(pdBody) && /res\.body\.getReader\(\)/.test(pdBody), "30.55: postDiscoveryTurn does not hold the turn POST and its SSE reader — that is the one place the four controls share");
    for (const control of ["#discovery-park", "#discovery-lookup", "#discovery-aside"])
      ok(new RegExp(`\\$\\('${control}'\\)\\.addEventListener`).test(js), `30.55: ${control} has no click handler — the control renders and does nothing`);
    ok(/\$\('#discovery-park'\)\.disabled = !answerable \|\| audit \|\| discovery\.running/.test(js), "30.55: the park button is not disabled in an audit — a park's reason is typed into #discovery-answer, which renderDiscoverySession hides on an audit, so an enabled button asks for something that is not on the page (and assertParkable refuses it anyway)");
    ok(/\$\('#discovery-offscript-row'\)\.hidden = audit/.test(js), "30.55: the off-script row is not hidden in an audit");
    const osAt = js.indexOf("function offScriptStatus(");
    const osBody = js.slice(osAt, js.indexOf("\n}", osAt));
    ok(osAt !== -1 && /session\.exchanges/.test(osBody), "30.55: the off-script status line does not read session.exchanges — it must report from the PACKAGE (a disk read), never from a count of stream events");
    ok(/\.turn === turn/.test(osBody), "30.55: the status line is not scoped to the OPEN turn — a package-wide read calls a turn that is simply still open \"unaccounted for\"");
    ok(!/answer the question on the table/i.test(osBody), "30.55: the status line tells the person to answer the question while an aside is unfiled — that is the action the applier now REFUSES, and a surface pointing at a refused action shows a claim the ops do not hold");
  }

  // 30.56 — the ROUTE, read DECOMMENTED so prose cannot stand in for a guard: /api/discovery/turn names
