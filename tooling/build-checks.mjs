...
// nobody can debug.
{
  ...
  // hold", so every rule below is a run against the applier's own output rather than a shape check.
  {
    ...
    // vacuous.
    {
      ...
      ok(lv.decisions.some((d) => !d.latest), "28.10: the cross-reader case is vacuous — the fixture must supersede something, or `visible` and `every decision` are the same set");
    }
