...
// discovery/README.md §The parenting fixture.
{
  ...
  if (pkg) {
    ...
    // 32.7 — the two RECORDED stamps pinned CURRENT-TO-LITERAL, read off the packages themselves. Belt
    ...
    for (const [slug, posture] of [["instrument-loans-1", "think"], ["graded-opus-a", "think-opus"], ["partner-audit-1", "grill"]]) {
      ...
      ok(same(stamps, [POSTURES[posture].fingerprint]), `32.7: discovery/${slug} carries ${JSON.stringify(stamps.map((x) => String(x).slice(0, 8)))} but the current ${posture} surface is ${POSTURES[posture].fingerprint.slice(0, 8)} — the prompt moved under a committed recording. partner-audit-1 is the one that makes Grill's stamp cost something, and it is why #289's DOMAIN_RULE went into Create-PRD's system prompt alone`);
